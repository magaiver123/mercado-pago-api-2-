...
export async function activateTotemService(input: ActivateTotemInput) {
  ...
  if (!activationCode || !deviceId) {
    throw new AppError("Código de ativação e Device ID são obrigatórios", 400)
  }
  ...
  if (existingByDevice?.status === "active") {
    throw new AppError("Este dispositivo já está ativado", 409)
  }

  const totem = await repositories.totem.findByActivationCode(activationCode)
  if (!totem) {
    throw new AppError("Código de ativação não encontrado", 404)
  }

  if (totem.status === "active") {
    throw new AppError("Código de ativação já utilizado", 409)
  }

  if (totem.device_id && totem.device_id !== deviceId) {
    throw new AppError("Código de ativação não aplicável para este dispositivo", 422)
  }
  ...
  if (activationResult === "conflict") {
    throw new AppError("Dispositivo já vinculado a outro totem", 422)
  }

  if (activationResult === "not_updated") {
    throw new AppError("Código de ativação já utilizado ou totem inativo", 409)
  }
